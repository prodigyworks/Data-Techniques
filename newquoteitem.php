				...
				for ($i = 0; $i < $header->itemCount(); $i++) {
					...
					if (! $item->deleted) {
						...
						if (($header->status == "N" || $header->status == "R") && $item->productdesc != "Expedite Charge" && $item->productdesc != "Emergency Charge") {
							echo "<img height=16 title='Edit item' src='images/edit.png' onclick='edit(\"" . $i . "\");' />";
						}
...
<div class="buttonContainer">
	...
	<?php
		if ($header->headerid != 0) {
	?>
			<input type="button" class="navButton" value="DOCUMENTS" onclick="viewDocument(<?php echo $header->headerid; ?>)"></input>
	<?php
		} else {
			
		}
	?>
	<input type="BUTTON" class="dataButton" id="btnAdditionalInfo" value="INSTALLATION INFO" />
	...
	<form method='post' action='savequoteheader.php' onsubmit="return proceed(this)">
	...
	<form method='post' action='newquotesummary.php' onsubmit="return proceed(this)">
		<input class="actionButton" type="SUBMIT" value="PROCEED" />
	</form>
</div>
...
<form class="contentform" id="quoteForm" name="quoteForm" method="post" action="savequote.php" onsubmit="return validate()">
	...
	<div>
		...
		<div id="bespoketask" style="display:none">
			<label>NOTES</label>
			<textarea id="bespokenotes" name="bespokenotes" cols=70 rows=11></textarea>
...
<script>
	...
	function showLongLineDestinations(qty) {
		...
		var areas = "<?php createComboOptions("id", "name", "datatech_areas", "WHERE siteid = " . $header->siteid); ?>";
	...
	function validate() {
		if ($("#category").find('option:selected').text() == "Copper" ||
			$("#category").find('option:selected').text() == "X-Connect") {
			if($("#coppercat1").val() == 0) {
			...
			$("#longlinepanelcategory").change(
					function() {
						...
						if ($("#longlinepanelcategory").find('option:selected').text() == "Copper Panels" ||
						    ...
						    $("#longlinepanelcategory").find('option:selected').text() == "X-Connect") {
							/* Copper. */
							getCategories(
									"#coppercat1",
