	...
	<table id="activeorders" class='grid list fixed' width=100% cellspacing=0 cellpadding=0>
		...
		<?php
			$qry = "SELECT A.*, DATE_FORMAT(A.createddate, '%d/%m/%Y %H:%i') AS createdate, D.prefix AS jobprefix, D.id AS jobid, " .
					"requiredbymode, B.firstname, B.lastname, C.name AS sitename " .
					"FROM datatech_quoteheader A " .
					"INNER JOIN datatech_jobheader D " .
					"ON D.quoteid = A.id " .
			...
			if ($result) {
				while (($member = mysql_fetch_assoc($result))) {
					...
					if ($member['requiredbymode'] == "N") {
						echo "<td>Non-rack dependant - (up to 5 racks worth - 5 days)</td>\n";
						
					} else if ($member['requiredbymode'] == "R") {
						echo "<td>Rack dependant - (up to 5 racks worth - 10 days)</td>\n";
						
					} else {
						echo "<td>TBA – Projects/bespoke – (Rack dependant over racks worth - 20 days)</td>\n";
					}
