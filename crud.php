...
//		print_r($parser->parsed);
		...
		foreach ($this->columns as $col) {
			if ($col['filter'] && isset($_POST['filter_' . $col['name']]) && $_POST['filter_' . $col['name']] != "") {
				...
				if ($col['type'] == "DATACOMBO" || $col['datatype'] == "user" || $col['datatype'] == "contact") {
...
//			}
		...
		foreach ($this->subapplications as $app) {
			...
			if ($okToRun) {
				if ($app['application'] != "") {
				?>
				   	<span  title="<?php echo $app['tooltip']; ?>"  class="wrapper"><a disabled class='subapp rgap2 link1' id="<?php echo $app['id']; ?>"  href="javascript:subApp('<?php echo $app['application']; ?>', getPK())"><em><b><img width=16 height=16 src='<?php echo $app['imageurl']; ?>' /> <?php echo $app['title']; ?></b></em></a></span>
				<?php
					
				} else {
				?>
				   	<span  title="<?php echo $app['tooltip']; ?>"  class="wrapper"><a disabled class='subapp rgap2 link1' id="<?php echo $app['id']; ?>"  href="javascript:<?php echo $app['script']; ?>(getPK())"><em><b><img width=16 height=16 src='<?php echo $app['imageurl']; ?>' /> <?php echo $app['title']; ?></b></em></a></span>
				<?php
				}
			}
		}
	...
	<script>
		...
		function crudDelete() {
			...
			callAjax(
					...
					function(data) {
						<?php
						if ($this->postDeleteEvent != null) {
							echo $this->postDeleteEvent . "(currentCrudID);\n";
						}
						?>
					}
				);
				...
				function() {
					...
					$("#editdialog").dialog({
							...
							open: function(event, ui){
								<?php
								if ($this->onOpenEditDialog != null) {
									echo $this->onOpenEditDialog . "();\n";
								}
								?>
							},
							buttons: {
