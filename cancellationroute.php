...
<div id="userDialog" class="modal">
	<form id="rolesForm" name="rolesForm" method="post">
		...
		<select class="listpicker" name="roles[]" multiple="true" id="roles" >
			<?php createComboOptions("roleid", "roleid", "datatech_roles", "", false); ?>
		</select>
	</form>
</div>
...
<?php
	if (isset($_POST['routerole'])) {
		...
		$qry = "DELETE FROM datatech_roleroutes WHERE roleid = '$routerole'";
		...
		for ($i = 0; $i < $counter; $i++) {
			...
			$qry = "INSERT INTO datatech_roleroutes (routeid, roleid) VALUES ('$routerole', '$role')";
			$result=mysql_query($qry);
		};
	}
	...
	function addRoute() {
		...
		$qry = "INSERT INTO datatech_cancellationroute (id) VALUES ('$role')";
		$result=mysql_query($qry);
	...
	function removeRoute() {
		$role = mysql_escape_string($_POST['pk1']);
		$qry = "DELETE FROM datatech_cancellationroute WHERE id = '$role'";
		$result=mysql_query($qry);
	...
	$qry = "SELECT * FROM datatech_cancellationroute";
