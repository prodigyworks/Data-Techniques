<?php
	...
	$qry = "SELECT A.*, DATE_FORMAT(A.approveddate, '%d/%m/%Y') AS approvaldate, A.customer, B.firstname, B.lastname, C.name " .
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('F1', 'EXPEDITED');
	$objPHPExcel->getActiveSheet()->SetCellValue('G1', 'EMERGENCY');
	$objPHPExcel->getActiveSheet()->SetCellValue('H1', 'COST');
	$objPHPExcel->getActiveSheet()->SetCellValue('I1', 'PM');
	$objPHPExcel->getActiveSheet()->SetCellValue('J1', 'COST CODE');
	$objPHPExcel->getActiveSheet()->SetCellValue('K1', 'SG-PO');
	$objPHPExcel->getActiveSheet()->SetCellValue('L1', 'SITE');
	$objPHPExcel->getActiveSheet()->SetCellValue('M1', 'QA\'d');
	$objPHPExcel->getActiveSheet()->SetCellValue('N1', 'RACK DEPENDANT');
	$objPHPExcel->getActiveSheet()->SetCellValue('O1', 'INVOICE Y/N');
	$objPHPExcel->getActiveSheet()->SetCellValue('P1', 'COMMENT');
	...
	while (($member = mysql_fetch_assoc($result))) {
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('D' . $row, $member['customer']);
		$objPHPExcel->getActiveSheet()->SetCellValue('E' . $row, $description);
		$objPHPExcel->getActiveSheet()->SetCellValue('F' . $row, ($expedite == true ? "Y" : " "));
		$objPHPExcel->getActiveSheet()->SetCellValue('G' . $row, ($emergency == true ? "Y" : " "));
		$objPHPExcel->getActiveSheet()->SetCellValue('H' . $row, ($expedite == true || $emergency ? (number_format($charge, 2, '.', '')) : ""));
		$objPHPExcel->getActiveSheet()->SetCellValue('I' . $row, $member['firstname'] . " " . $member['lastname']);
		$objPHPExcel->getActiveSheet()->SetCellValue('J' . $row, getCostCodeName($member['costcode']));
		$objPHPExcel->getActiveSheet()->SetCellValue('K' . $row, $member['sungardpo']);
		$objPHPExcel->getActiveSheet()->SetCellValue('L' . $row, $member['name']);
		$objPHPExcel->getActiveSheet()->SetCellValue('M' . $row, $member['status'] == 'Q' ? "Y" : " ");
		$objPHPExcel->getActiveSheet()->SetCellValue('N' . $row, ($member['requiredbymode'] == "R" ? "Y" : ($member['requiredbymode'] == "N" ? "N" : "TBA")));
		...
		if ($member['sungardpo'] == null || trim($member['sungardpo']) == "") {
			...
			$objPHPExcel->getActiveSheet()->getStyle('K' . $row)->getFill()->getStartColor()->setARGB('FFFF0000');
		...
		$objPHPExcel->getActiveSheet()->getStyle('P' . $row)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
		$objPHPExcel->getActiveSheet()->getStyle('P' . $row)->getFill()->getStartColor()->setARGB($colourArray[$colour]);
	}


	$objPHPExcel->getActiveSheet()->getStyle('A1:P' . $row)->applyFromArray($styleArray);
	$objPHPExcel->getActiveSheet()->getStyle('A1:P1')->applyFromArray($headerArray);
	...
	for ($i = 0; $i < 5; $i++) {
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('H' . ($row + ($i + 2)), number_format($weeklyTotalArray[$i]['charge'], 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('F' . ($row + ($i + 2)), $weeklyTotalArray[$i]['expedite']);
		$objPHPExcel->getActiveSheet()->SetCellValue('G' . ($row + ($i + 2)), $weeklyTotalArray[$i]['emergency']);
		$objPHPExcel->getActiveSheet()->SetCellValue('K' . ($row + ($i + 2)), $weeklyTotalArray[$i]['missingpo']);
		$objPHPExcel->getActiveSheet()->getStyle('A' . ($row + ($i + 2)) . ':P' . ($row + ($i + 2)))->applyFromArray($headerArray);
		...
		$objPHPExcel->getActiveSheet()->getStyle('H' . ($row + ($i + 2)))->getFont()->getColor()->setARGB(PHPExcel_Style_Color::COLOR_RED);
		$objPHPExcel->getActiveSheet()->getStyle('K' . ($row + ($i + 2)))->getFont()->getColor()->setARGB(PHPExcel_Style_Color::COLOR_RED);
		...
		$objPHPExcel->getActiveSheet()->getStyle('H' . ($row + ($i + 2)))->getFill()->getStartColor()->setARGB($colourArray[$i]);
		$objPHPExcel->getActiveSheet()->getStyle('H' . ($row + ($i + 2)))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
		...
		$objPHPExcel->getActiveSheet()->getStyle('K' . ($row + ($i + 2)))->getFill()->getStartColor()->setARGB($colourArray[$i]);
		$objPHPExcel->getActiveSheet()->getStyle('K' . ($row + ($i + 2)))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
	}
	
	$objPHPExcel->getActiveSheet()->SetCellValue('A' . ($row + 9), "Totals");
	$objPHPExcel->getActiveSheet()->SetCellValue('C' . ($row + 9), "=SUM(B1:B" . $row . ")");
	$objPHPExcel->getActiveSheet()->SetCellValue('H' . ($row + 9), "=SUM(F1:F" . $row . ")");
	$objPHPExcel->getActiveSheet()->SetCellValue('F' . ($row + 9), $numberExpedite);
	$objPHPExcel->getActiveSheet()->SetCellValue('G' . ($row + 9), $numberEmergency);
	$objPHPExcel->getActiveSheet()->SetCellValue('K' . ($row + 9), $missingPO);
	$objPHPExcel->getActiveSheet()->getStyle('A' . ($row + 9) . ':P' . ($row + 9))->applyFromArray($headerArray);
	...
	$objPHPExcel->getActiveSheet()->getStyle('H' . ($row + 9))->getFont()->getColor()->setARGB(PHPExcel_Style_Color::COLOR_RED);
	$objPHPExcel->getActiveSheet()->getStyle('K' . ($row + 9))->getFont()->getColor()->setARGB(PHPExcel_Style_Color::COLOR_RED);
