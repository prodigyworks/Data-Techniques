<?php
	...
	function saveContent() {
		...
		$qry = "UPDATE {$_SESSION['DB_PREFIX']}pages SET content = '$contentvalue' WHERE pageid = $pageid";
		$result = mysql_query($qry);
	...
	class PageEdit extends Crud {
		...
		public function postInsertEvent() {
			...
			$qry = "INSERT INTO {$_SESSION['DB_PREFIX']}pageroles (pageid, roleid) VALUE($pageid, 'PUBLIC')";
			$result = mysql_query($qry);
			...
			if ($_POST['menuitem'] == "Y") {
				$qry = "INSERT INTO {$_SESSION['DB_PREFIX']}pagenavigation (pageid, childpageid, sequence, pagetype) VALUE($parentpageid, $pageid, $maxSequence, 'P')";
				
			} else {
				$qry = "INSERT INTO {$_SESSION['DB_PREFIX']}pagenavigation (pageid, childpageid, sequence, pagetype) VALUE($parentpageid, $pageid, $maxSequence, 'L')";
			}
