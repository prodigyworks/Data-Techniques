...
//	       $image = mysql_real_escape_string($image);
	       ...
	       if (isset($_GET['id'])) $headerid = $_GET['id'];
	       if (isset($_GET['sessionid'])) $sessionid = $_GET['sessionid'];
	       
//	       mysql_real_escape_string
			$stmt = mysqli_prepare($link, "INSERT INTO datatech_documents " .
					"(headerid, sessionid, name, filename, mimetype, image, size, createdby, createddate) " .
					"VALUES " .
					"(?, ?, ?, ?, ?, ?, ?, ?, NOW())");
			...
			mysqli_stmt_bind_param($stmt, "isssssss", $headerid, $sessionid, $description, $filename, $mimetype, $image, $size, $_SESSION['SESS_MEMBER_ID']);
